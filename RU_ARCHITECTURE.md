...
1. `SignalWorker` получает сигнал ENTRY
...
4. Worker устанавливает `trade.stop_loss`, `trade.stoploss`, `trade.stop_loss_pct`
5. Worker проверяет SL относительно цены ликвидации (обрезает при необходимости)
6. Worker размещает **TP лимитный ордер** на бирже через прямой CCXT API
...
### Срабатывание SL (биржа исполняет SL триггер)
...
### 3. Предупреждения о несовпадении объёма
- BingX иногда возвращает агрегированные объёмы с небольшим отклонением
- `WARNING: Amount X does not match amount Y` — безопасно игнорировать
- Не влияет на закрытие сделки или расчёт прибыли
...
| Орфан-сделка (нет позиции на бирже) | Reconcile пропускает, `handle_onexchange_order` закрывает | Авто-очистка |
