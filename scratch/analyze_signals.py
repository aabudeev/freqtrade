...
def analyze_signals():
    ...
    print("\n--- Raw Signal Analysis ---")
    print(f"{'Signal Text Snippet':<40} | {'Entry':<8} | {'Stop':<8} | {'Dist %':<8} | {'Profit %'}")
    print("-" * 95)
    ...
    for (text,) in messages:
        ...
        if price_match and stop_match and target_match:
            ...
            profit_dist = abs(target_price - avg_entry) / avg_entry
            
            snippet = text[:40].replace('\n', ' ')
            print(f"{snippet:<40} | {avg_entry:<8.4f} | {stop_price:<8.4f} | {dist*100:<7.2f}% | {profit_dist*100:<8.2f}%")
