...
- **Восстановлены защиты**: 
  - Проверка спреда перед входом (не более 2%).
  - Проверка `entry_range` (буфер ±0.5% для межбиржевой разницы, до 10 попыток с интервалом 1 мин).
  - Emergency Exit (если текущая цена уже за SL при входе).
  - Авто-подстановка SL (2.5%) и TP (3.5%) если не указаны в сигнале.
  - Точный P&L для сделок, закрытых на бирже (Order History -> Ticker -> Open Rate).
